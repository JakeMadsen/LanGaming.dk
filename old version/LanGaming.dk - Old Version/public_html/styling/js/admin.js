/* Online Host and Port */
// const   host = 'http://185.134.28.169:';
// const   port = '80';
...
function postStudentCheckIn(){
    ...
    if( check_name == "" || check_phone == "" || check_cpr == "" || check_email == "" || check_elevPlan == "" || check_eventId == ""){
        ...
        let request = new Request(host + port + '/json/checkIn/new', init);
                
        fetch(request)
            .then(response => 
                {
                    console.log("Succes",response)
                    window.location=(host + port + '/cPanel/checkIn');
                })
            .catch(err => 
                {   
...
function getLendTypes() {
    ...
    fetch(host + port + '/json/lend/type/all')
        .then(function (data) 
        {
            return data.json();
        })
...
function postNewLend(){
    ...
    if(student_name == "" || cable_id == null){
        ...
        let request = new Request(host + port + '/json/lend/new', init);
                    
        fetch(request)
            .then(response => 
                {
                    console.log("Succes",response)
                    window.location=(host + port + '/cPanel/lend');
                })
            .catch(err => 
                {   
                    console.log("Fejlbesked",err, request)
...
function deleteLend(lend_id){
    ...
    if (confirmDelete == true) {
        ...
        let request = new Request(host + port + '/json/lend/delete', init);
...
function postNewLendType(){
    ...
    if(new_lend_type_name == ""){
        ...
        let request = new Request(host + port + '/json/lend/type/new', init);
...
function editLendType(){
    ...
    if(new_lend_type_name == ""){
        ...
        let request = new Request(host + port + '/json/lend/type/edit', init);
    ...
    if (confirmDelete == true) {
        checkIfLendTypeIsInUse(lend_type_id, function(checkIfInUse){
            ...
            if(checkIfInUse == true){
                ...
                let request = new Request(host + port + '/json/lend/type/delete', init);
